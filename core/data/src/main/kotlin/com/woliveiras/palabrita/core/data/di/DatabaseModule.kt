...
object DatabaseModule {
    ...
    fun provideDatabase(@ApplicationContext context: Context): PalabritaDatabase =
        Room.databaseBuilder(context, PalabritaDatabase::class.java, "palabrita.db").build()

    @Provides fun providePuzzleDao(db: PalabritaDatabase): PuzzleDao = db.puzzleDao()

    @Provides
    fun providePlayerStatsDao(db: PalabritaDatabase): PlayerStatsDao = db.playerStatsDao()

    @Provides
    fun provideGameSessionDao(db: PalabritaDatabase): GameSessionDao = db.gameSessionDao()

    @Provides
    fun provideChatMessageDao(db: PalabritaDatabase): ChatMessageDao = db.chatMessageDao()

    @Provides
    fun provideModelConfigDao(db: PalabritaDatabase): ModelConfigDao = db.modelConfigDao()
}
