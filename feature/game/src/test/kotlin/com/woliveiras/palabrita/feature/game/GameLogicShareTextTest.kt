...
class GameLogicShareTextTest {
  ...
  fun `share text maps CORRECT to blue square`() {
    val attempts = listOf(Attempt("gatos", List(5) { LetterFeedback('g', LetterState.CORRECT) }))
    val text =
      GameLogic.generateShareText(
        attempts = attempts,
        difficulty = 1,
  ...
  fun `share text maps PRESENT to orange square`() {
    val attempts = listOf(Attempt("togsa", List(5) { LetterFeedback('t', LetterState.PRESENT) }))
    val text =
      GameLogic.generateShareText(
        attempts = attempts,
        difficulty = 1,
  ...
  fun `share text maps ABSENT to red square`() {
    val attempts = listOf(Attempt("xxxxx", List(5) { LetterFeedback('x', LetterState.ABSENT) }))
    val text =
      GameLogic.generateShareText(
        attempts = attempts,
        difficulty = 1,
  ...
  private fun createWinAttempts(target: String, count: Int): List<Attempt> {
    ...
    val winAttempt =
      Attempt(word = target, feedback = target.map { LetterFeedback(it, LetterState.CORRECT) })
